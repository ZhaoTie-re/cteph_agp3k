...
def plot_vmiss_distribution_by_maf_category(
    ...
    for ax, category, color in zip(axes, categories, colors):
        ...
        # 绘制带核密度估计的直方图
        sns.histplot(sub_df["VMISS"], bins=bin_edges, kde=True, color=color, # type: ignore
    ...
    plt.savefig(f"{output_prefix}.vmiss.png", dpi=600) # 保存为PNG格式（PDF太大）
    plt.show()
...
def plot_hwe_scatter_by_maf_category(
    ...
    plt.savefig(f"{output_prefix}.hwe.png", dpi=600) # 保存为PNG格式（PDF太大）
    plt.show()
