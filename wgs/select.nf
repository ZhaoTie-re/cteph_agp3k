...
// PHOM0518 & PHOM0582 are ambiguous samples, remove them before sample QC
// Using plink2 to remove samples
// PHOM0518 & PHOM0582 response for IID column in fam file
// fam file from plink2 output does not have column name, so just focus on second column

process removeAmbiguousSamples {
    executor 'slurm'
    queue 'gr10478b'
    time '6h'
    tag "removeAmbiguousSamples"
    ...
    output:
    tuple file("*.bed"), file("*.bim"), file("*.fam") into clean_samples_out

    script:
    out_prefix = "cteph_agp3k.autosome.gt_qc.clean"
    """
    # Create a remove list file with FID and IID
    echo "PHOM0518\tPHOM0518" > remove_samples.txt
    echo "PHOM0582\tPHOM0582" >> remove_samples.txt
...
process RunSampleQC {
    ...
    input:
    tuple file(bed), file(bim), file(fam) from clean_samples_out
...
process FinishSampleQC {
    ...
    output:
    file('*.log')
    tuple file("*.bed"), file("*.bim"), file("*.fam") into finish_sample_qc_out, finish_sample_qc_out_2
...
process RunPCA_rev {
    ...
    publishDir "${params.outdir}/12.run_pca_rev", mode: 'symlink'
...
process BBJSampleKeep_rev {
    executor 'slurm'
    queue 'gr10478b'
    time '36h'
    tag "BBJSampleKeep"

    publishDir "${params.outdir}/13.bbj_sample_keep", mode: 'symlink'

    input:
    tuple file(bed), file(bim), file(fam) from finish_sample_qc_out_2
    ...
    python ${params.scriptDir}/bbj_sample_keep_main.py \
        ...
        --rect_xlim -0.030 0.016 \
        --rect_ylim -0.025 0.033 \
        --bed_prefix ${bed_prefix} \
        --threads 32
    """
}
...
process rmMAF0orVMISS {
    ...
    publishDir "${params.outdir}/14.rm_maf0_vmiss", mode: 'symlink'
...
process ToMMoPanelFilter_rev {
    ...
    python ${params.scriptDir}/panel_filter_main.py \
        ...
        --bed_prefix    ${bed_prefix} \
        --out_prefix    cteph_agp3k \
        --threads 16 --chunk_size 500000 --max_workers 10 \
        --keep_tmp \
        --merge_low_common \
        --plink2_path /home/b/b37974/plink2 \
        --save_out_map
